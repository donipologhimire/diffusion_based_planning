...
GUIDANCE_SCALE = 4.0 # Tune this value (e.g., 1.0 = no guidance, higher values = stronger conditioning)
...
def generate_trajectory_visualization(model_path, output_image_path, guidance_scale=1.0):
    """Generate and visualize a trajectory using the model checkpoint at model_path"""
    # --- Load Model ---
    model = ConditionalUNet1D(
        ...
        cond_emb_extra_dim=MODEL_COND_EMB_EXTRA_DIM,
        attn_levels=MODEL_ATTN_LEVELS
    ).to(DEVICE)
    ...
    # --- Define start/goal and obstacles (same for all) ---
    ...
    # --- Generate trajectory ---
    sampling_shape = (1, MAX_TRAJ_LEN, STATE_DIM)
    generated_traj_norm = p_sample_loop(
        ...
        verbose=False  # Disable tqdm progress bars during batch processing
    )
    ...
    plt.plot(path_cols + 0.5, path_rows + 0.5, marker='.', color='cyan', linestyle='-', linewidth=2, markersize=5)
    plt.plot(start_point_grid[1] + 0.5, start_point_grid[0] + 0.5, 'go', markersize=12, markeredgecolor='black')
    plt.plot(goal_point_grid[1] + 0.5, goal_point_grid[0] + 0.5, 'ro', markersize=12, markeredgecolor='black')
    
    plt.title(f"Epoch {epoch_num} - Guidance Scale: {guidance_scale}")
    ...
    return epoch_num, output_image_path
...
# Add this parameter to disable tqdm during batch processing
@torch.no_grad()
def p_sample_loop(model, shape, condition_sg, condition_obs, start_coord_norm, goal_coord_norm, guidance_scale, verbose=True):
    """Performs the full reverse diffusion sampling loop with CFG"""
    batch_size = shape[0]
    x_t = torch.randn(shape, device=DEVICE)
    ...
    # Use tqdm only when verbose=True
    if verbose:
        print(f"Starting sampling loop with Guidance Scale: {guidance_scale}...")
        timesteps = tqdm(reversed(range(0, N_DIFFUSION_STEPS)), desc="Sampling", total=N_DIFFUSION_STEPS)
    else:
        timesteps = reversed(range(0, N_DIFFUSION_STEPS))
        
    for i in timesteps:
        t = torch.full((batch_size,), i, device=DEVICE, dtype=torch.long)
        x_t = p_sample(model, x_t, t, i, condition_sg, condition_obs, null_sg, null_obs, guidance_scale)
        
        # Apply Start/Goal Constraints at each step
        x_t[:, 0, :] = torch.tensor(start_coord_norm, dtype=torch.float32, device=DEVICE)
        x_t[:, -1, :] = torch.tensor(goal_coord_norm, dtype=torch.float32, device=DEVICE)
...
def create_trajectory_evolution_gif(checkpoints_dir, output_gif_path, guidance_scale=1.0):
    """Create a GIF showing trajectory evolution across model checkpoints"""
    # Create a temp directory for frame images
    temp_dir = "temp_frames"
    os.makedirs(temp_dir, exist_ok=True)
    
    # Find all checkpoint files, sort them by epoch number
    ...
    for i, checkpoint_path in enumerate(sorted(checkpoint_files, key=lambda x: int(Path(x).stem.split('_')[-1]))):
        print(f"Processing checkpoint {i+1}/{len(checkpoint_files)}: {checkpoint_path}")
        frame_path = os.path.join(temp_dir, f"frame_{i:03d}.png")
        
        # Generate trajectory visualization for this checkpoint
        result = generate_trajectory_visualization(checkpoint_path, frame_path, guidance_scale)
        
        if result:
            epoch_num, _ = result
            processed_frames.append((epoch_num, frame_path))
    
    # Sort frames by epoch number
    processed_frames.sort(key=lambda x: x[0])
    frame_paths = [frame for _, frame in processed_frames]
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Installing imageio package...")
        import subprocess
        subprocess.check_call([sys.executable, "-m", "pip", "install", "imageio"])
        import imageio
    
    # Default checkpoint path (single model)
    MODEL_CHECKPOINT_PATH = 'diffusion_policy_unet_refined_cfg.pth'
    
    # Parameters for the GIF generation
    CHECKPOINTS_DIR = "checkpoints"  # Directory containing epoch checkpoints
    OUTPUT_GIF_PATH = "trajectory_evolution.gif"
    GUIDANCE_SCALE = 1.0  # Adjust as needed
    
    # Choose mode
    import sys
    if len(sys.argv) > 1 and sys.argv[1] == '--gif':
        # Create evolution GIF from all checkpoints
        create_trajectory_evolution_gif(CHECKPOINTS_DIR, OUTPUT_GIF_PATH, GUIDANCE_SCALE)
    else:
        # Regular inference with a single model
        ...
        plt.imshow(obstacle_map, cmap='gray_r', origin='lower', extent=(0, GRID_SIZE, 0, GRID_SIZE), alpha=0.8)
        path_rows, path_cols = final_trajectory_denorm[:, 0], final_trajectory_denorm[:, 1]
        plt.plot(path_cols + 0.5, path_rows + 0.5, marker='.', color='cyan', linestyle='-', linewidth=2, markersize=5, label='Generated Trajectory (CFG)')
